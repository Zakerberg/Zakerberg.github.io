...
date: 2026-07-22
top: true
description: 从 S31、簧片、M4/M5/M6、M7、M8、M9 到 MX、ML、MY、MV、MX Low Profile 与 MX Ultra Low Profile，系统整理 Cherry 键盘开关的历史、结构、手感和技术参数。
...
本文以 [Telcontar 的 Cherry Switches 档案](https://telcontar.net/KBK/Cherry/#switches) 为主线，并结合其收录的 Cherry 原厂图纸、目录、专利和数据表进行中文整理。文中的年份、参数与型号均尽量保留资料边界：有争议的地方会明确标注“推测”“约”或“待考”，不会把收藏圈经验当成原厂结论。

> **图片与资料使用说明：**本文是中文整理，不是 CHERRY 官方资料。图片仅选用 Telcontar 声明为公共领域的站内作品，或页面明确标为 CC0 的照片；不使用其 Reference 区照片、品牌 Logo 及标注为“经许可使用”的第三方图片。原始数据仍请以对应年代的 Cherry 图纸、目录和数据表为准。
...
| MX | 1983 年末起 | Gold Crosspoint | 通常 4 mm | 最长寿、影响最大的 Cherry 独立机械轴系列 |
...
| MX Low Profile | 2018 年 | 低矮 Gold Crosspoint | 3.2 mm | 独立板装低矮轴，Red 与 Speed 两类 |
| MV / Viola | 2020 年 | 开放式桥接触点 | 4.0 mm | 面向主流价位的半独立结构 |
| MX Ultra Low Profile | 2021 年 | 开放式 SMD 结构 | 1.8 mm | 3.5 mm 高，面向笔记本及超薄设备 |
...
## MX Low Profile：独立低矮机械轴

MX Low Profile 于 2018 年 1 月 12 日发布。它沿用金合金交叉点理念，但并不是把普通 MX 简单压矮，而是一套新结构。早期资料记录两个线性版本：红色轴心 `MX1B-L2NA` 和银色 Speed `MX1B-52NA`，都为外置 SMD RGB LED 预留空间。
...
## MX Ultra Low Profile：3.5 mm 高的超薄轴

MX Ultra Low Profile 于 2021 年 3 月 18 日发布，面向笔记本和超薄设备。它不再是传统封闭轴壳，而是高度约 `3.5 mm` 的开放式 SMD 结构；两只摆臂在跨接弹簧作用下运动，中部透明执行件承载键帽并允许 SMD RGB 光线通过。
...
最后更新：2026-07-22。后续如发现新的原厂目录、图纸或可核对实物，本文会继续修订，并保留不确定信息的说明。
